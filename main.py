...
# رابط الأساس لملفات الـJSON في GitHub
# تأكد من المسار الصحيح لمستودعك هنا:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    2) يعرض قائمة المواضيع الرئيسية (Topic) كأزرار.
    """
    ...
    try:
        ...
        topics_data = resp.json()  # يفترض أن يحتوي على مفتاح "topics"
        # استخراج المصفوفة من المفتاح "topics"
...
async def receive_question_count(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        resp.raise_for_status()
        questions_list = resp.json()  # يفترض أن يكون مصفوفة من الكائنات (الأسئلة)
    except Exception as e:
    ...
    for idx, qdata in enumerate(selected_questions, start=1):
        ...
        sent_poll = await context.bot.send_poll(
            ...
            open_period=120,  # يمكنك تعديلها أو إزالتها
            parse_mode="HTML"
        )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            QUIZ_IN_PROGRESS: [
                # لا نضع معالجات هنا لأن إجابات الاستفتاء تُعالج عبر PollAnswerHandler
            ],
        },
